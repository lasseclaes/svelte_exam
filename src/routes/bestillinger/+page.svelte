  ...
  {#each tabItems as tabItem, i}
    <li class="nav-item" role="presentation">
      <button
        ...
        id="{tabItem.slug}-tab"
        data-bs-toggle="tab"
        data-bs-target="#{tabItem.slug}-tab-pane"
        type="button"
        role="tab"
        aria-controls="{tabItem.slug}-tab-pane"
        aria-selected="true"
        >{tabItem.title}
...
<div class="tab-content" id="myTabContent">
  {#each tabItems as tabItem, tabindex}
    <div
      class="tab-pane fade"
      id="{tabItem.slug}-tab-pane"
      role="tabpanel"
      aria-labelledby="{tabItem.slug}-tab"
      tabindex={tabindex + 1}
    >
      <Table filter={tabItem.slug} {tabindex} />
    </div>
  {/each}
</div>
